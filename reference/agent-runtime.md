...
```json
{
  ...
  "phase": "estimate",
  "requested_coverage": "source_exhaustive"
}
```
...
- `discover_missing`：只发现当前库里不存在的三元组，只输出 `new_candidates`；`update_candidates`、`conflicts`、`unchanged_examples` 必须为空。
...
```json
{
  ...
  "phase": "batch",
  "relation_ids": ["relation-id-1", "relation-id-2"],
  "batch_index": 1,
  "batch_size": 20
}
```

- `discover_missing phase=estimate`：估算候选总量、已有数量、缺失数量、严格交叉验证并推荐运行策略，不返回候选。
- `discover_missing phase=batch`：按 `max_candidates` 返回本批缺失候选。
- `verify_existing_only phase=plan`：规划或核对存量验证批次，不返回候选。
- `verify_existing_only phase=batch`：只验证输入 `relation_ids` 对应的存量关系。
...
- 如输入包含 `relation_ids`，只验证这些关系，不得扩展到其他关系。
- 对大量存量关系，应按 `relation_ids` 分批验证。
...
- `phase=estimate` 只做规模估算和策略建议，不返回 `new_candidates`。
- `phase=batch` 每次都重新查询现有 `published` 和 `pending_review` 实例，再返回缺失候选。
- 只输出现有基线中不存在的三元组。
- 如果发现的是已有三元组的更好说明或更强来源，本模式应跳过，不输出 `update_candidates`。
- 如果发现已有三元组与新证据矛盾，本模式也不得输出 `conflicts`；只可在 `self_check.notes` 或 `coverage.notes` 中提示应另跑 `verify_existing_only`。
- 不输出 `unchanged_examples`，因为本模式不负责存量验真。
- 对可枚举关系类型，按关系文档的主来源构造候选全集并 diff。
- 对开放关系类型，只输出本轮高置信发现，并将覆盖状态标记为 `partial`。
- 对关系量很大的关系类型，不要求全量找完；应自行收窄到热门球员、热门球队、报道较多或问答价值最高的范围，并在 `estimate.scope_note` 和 `coverage.notes` 中声明覆盖口径。
- 估算必须严格交叉验证，至少说明主权威来源、数据库已有唯一关系数、辅助来源或预期范围。
...
- `Estimate Workflow`：`discover_missing phase=estimate` 的数量估算与交叉验证流程。
- `Verify Workflow`：`verify_existing_only phase=batch` 的存量核验流程。
- `Run Policy`：推荐的 `max_candidates`、`max_batches`、`batch_size`、热门覆盖口径。
